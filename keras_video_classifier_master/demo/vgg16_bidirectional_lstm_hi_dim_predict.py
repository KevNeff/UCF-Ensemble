...
def main():
    ...
    ensemble = True
    if ensemble == True:
        pFile = open('lstmpredictions.txt', 'w')
        for video_file_path in video_file_path_list:
            prediction = predictor.predict_values(video_file_path)
            for softmax in prediction:
                pFile.write(str(softmax) + ", ")
            pFile.write(str(predictor.get_index(videos[video_file_path])) + "\n")
        pFile.close()
    else:
        for video_file_path in video_file_path_list:
