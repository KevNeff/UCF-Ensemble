...
class VGG16BidirectionalLSTMVideoClassifier(object):
    ...
    def load_model(self, config_file_path, weight_file_path):
        ...
        self.labels_word2idx = dict(zip(self.labels_idx2word.values(),self.labels_idx2word.keys()))
        self.config = config
    ...
    def predict_values(self, video_file_path):
        x = extract_vgg16_features_live(self.vgg16_model, video_file_path)
        frames = x.shape[0]
        if frames > self.expected_frames:
            x = x[0:self.expected_frames, :]
    ...
    def get_index(self, word_label):
        return self.labels_word2idx[word_label]
...
class VGG16LSTMVideoClassifier(object):
    ...
    def fit(self, data_dir_path, model_dir_path, vgg16_include_top=True, data_set_name='UCF-101', test_size=0.2, random_state=43):
        self.vgg16_include_top = vgg16_include_top
